...
class CacheBuster:
    def __init__(self, app=None, config=None):
        ...
        self.hash_size = self.config.get(
            'hash_size') if self.config else HASH_SIZE
    ...
    def init_app(self, app, config=None):
        ...
        for dirpath, dirnames, filenames in os.walk(app.static_folder):
            for filename in filenames:
                ...
                if not self.__is_file_to_be_busted(rooted_filename):
                    continue
                app.logger.debug(
                    'Computing hashes for {}'.format(rooted_filename))
        ...
        def bust_filename(file):
            return bust_map.get(os.path.normpath(file), file)

        def unbust_filename(file):
            return unbust_map.get(os.path.normpath(file), file).replace(os.sep, "/")
